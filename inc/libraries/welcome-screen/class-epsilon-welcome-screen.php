...
class Epsilon_Welcome_Screen{
	...
	public function __construct( $config = [] ) {
		 ...
		 */
		add_action( 'admin_menu', [ $this, 'welcome_screen_menu' ] );
		 ...
		 * Ajax callbacks
		 */
		add_action( 'wp_ajax_welcome_screen_ajax_callback', [
			$this,
			'welcome_screen_ajax_callback',
		] );
		add_action( 'wp_ajax_nopriv_welcome_screen_ajax_callback', [
			$this,
			'welcome_screen_ajax_callback',
		] );
		...
		if ( $this->edd ) {
			 ...
			 * Initiate EDD Stuff
			 */
			add_action( 'admin_init', [ 'EDD_Theme_Helper', 'init' ] );
	...
	private function count_actions() {
		$actions_left = get_option( $this->theme_slug . '_actions_left', [] );
	 ...
	 * @return array
	 */
	private function set_default_sections( $config = [] ) {
		$arr = [
			'getting-started'     => [
				'id'    => 'getting-started',
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= sprintf( /* Translators: Notice */
				esc_html__( 'Welcome! Thank you for choosing %3$s! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'epsilon-framework' ), '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome' ) ) . '">', '</a>', $this->theme_name );
			$this->notice .= '</p>';
	 ...
	 * @return string;
	 */
	public static function handle_required_action( $args = [] ) {
		$instance     = self::get_instance();
		$actions_left = get_option( $instance->theme_slug . '_actions_left', [] );
	 ...
	 * @return string;
	 */
	public static function set_frontpage_to_static( $args = [] ) {
	...
	private function get_plugins_left() {
		if ( ! empty( $this->plugins ) ) {
			$plugins_left = get_option( $this->theme_slug . '_plugins_left', [] );
	...
	private function check_plugin( $slug = '' ) {
		...
		if ( in_array( $slug, [ 'contact-form-7' ] ) ) {
			switch ( $slug ) {
				case 'contact-form-7':
	...
	private function create_plugin_link( $state, $slug ) {
		...
		switch ( $state ) {
			case 'install':
				$string = wp_nonce_url( add_query_arg( [
					'action' => 'install-plugin',
					'plugin' => Epsilon_Notify_System::_get_plugin_basename_from_slug( $slug ),
				], network_admin_url( 'update.php' ) ), 'install-plugin_' . $slug );
				break;
			case 'deactivate':
